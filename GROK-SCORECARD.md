...
| Scroll scrubs the film (`scrub: 1`) | Done — Lenis + GSAP ScrollTrigger on `video.currentTime` |
| No mouse-look | Done |
| Poster first, LCP | `media/drive-poster.jpg` (~143KB), then swap to film |
| Hero weight | Poster 0.14MB + film 5.5MB = **5.6MB** (under 8MB) |
| Reduced motion | Static poster, no pin, no Lenis, no fades |
| No-video fallback | Poster stays; type readable |
